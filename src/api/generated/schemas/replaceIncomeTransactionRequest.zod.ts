/**
 * Generated by orval v8.23.0 🍺
 * Do not edit manually.
 * Core Console API
 * Initial backend-owned API definition for Core Console. Operational endpoints are intentionally excluded.
 * OpenAPI spec version: 0.1.0
 */
import { z as zod } from "zod";

export const replaceIncomeTransactionRequestCategoryAllocationsMax = 1;

export const ReplaceIncomeTransactionRequest = zod
  .strictObject({
    accountId: zod.uuid(),
    categoryAllocations: zod
      .array(
        zod
          .strictObject({
            amount: zod
              .strictObject({
                amount: zod
                  .string()
                  .describe(
                    "Plain base-10 decimal string. After sign and insignificant leading zeros are normalized, a non-zero value may have at most 131,072 integer digits; currency minor-unit precision applies separately. Values outside this durable PostgreSQL numeric range return 422 validation_error.",
                  ),
                currency: zod.enum(["CNY", "JPY", "USD"]),
              })
              .describe(
                "Exact decimal-string Money supplied by a Finance caller.",
              ),
            categoryId: zod.union([zod.uuid(), zod.null()]).optional(),
          })
          .describe("One complete v1 allocation, optionally Uncategorized."),
      )
      .min(1)
      .max(replaceIncomeTransactionRequestCategoryAllocationsMax),
    economicAmount: zod
      .strictObject({
        amount: zod
          .string()
          .describe(
            "Plain base-10 decimal string. After sign and insignificant leading zeros are normalized, a non-zero value may have at most 131,072 integer digits; currency minor-unit precision applies separately. Values outside this durable PostgreSQL numeric range return 422 validation_error.",
          ),
        currency: zod.enum(["CNY", "JPY", "USD"]),
      })
      .describe("Exact decimal-string Money supplied by a Finance caller."),
    kind: zod.literal("income"),
    note: zod.union([zod.string(), zod.null()]).optional(),
    transactionDate: zod.iso.date(),
  })
  .describe("Completely replace one existing Income.");

export type ReplaceIncomeTransactionRequest = zod.input<
  typeof ReplaceIncomeTransactionRequest
>;
export type ReplaceIncomeTransactionRequestOutput = zod.output<
  typeof ReplaceIncomeTransactionRequest
>;
